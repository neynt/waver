...
let midi_demo input_file output_file =
  ...
  let offsets_and_signals =
    List.map (List.concat midi_file.tracks) ~f:(fun { midi; time; dur; velo; _ } ->
      (time *. time_scale), Instrument.blip midi (dur *. time_scale) ~velo:(Float.of_int velo /. 255.))
  in
...
let frame_demo input_file =
  ...
  let track =
    ...
    |> List.sort ~compare:(fun { Midi. time = time1; _ } { time = time2; _ } -> Float.compare time1 time2)
  in
  ...
  let max_time =
    List.fold track ~init:0. ~f:(fun max { time; dur; _ } -> Float.max max (time +. dur))
  in
  let t_to_y t =
    1080 - 150 -
    (Int.of_float
  ...
  let save_frame time_ofs filename =
    ...
    Map.iteri midi_file.key_signatures ~f:(fun ~key ~data ->
      ...
      let scale = Scale.from_accidental_count ~quality:key_signature.quality key_signature.accidentals in
      for midi = min_note to max_note do
        ...
        | `Natural -> ()
        | _ ->
          Draw.(striped_rect frame (x + 1) end_y (semitone_width - 2) h 5 (rgb 48 48 48) (rgb 16 16 16))
      done);
    ...
    List.iter track ~f:(fun { midi; time; dur; _ } ->
      ...
      let style: Draw.rect_style =
        ...
        | None -> Flat color
        | Some deg' ->
          let color' = deg_to_color (deg' % Array.length scale.offsets) quality in
          Striped (5, color, color')
      in
      Draw.rect_gen frame (x + 1) end_y (x' - x - 2) h style;
      Draw.(rect frame (x + 1) (start_y - 1) (x' - x - 2) (Int.min 1 h) (rgb 16 16 16));
    ...
    let _ = Draw.piano
      ~semitone_width
      ~white_color:(chr_to_col white_color)
      ~black_color:(chr_to_col black_color)
...
let video save_frame max_time output_dir fps =
  ...
  let rec loop frame =
    ...
    | true -> ()
    | false ->
      save_frame time_ofs (Core.Filename.concat output_dir (Caml.Printf.sprintf "%05d.png" frame));
      loop (frame + 1)
  in
  loop 0
;;
...
let render_cmd =
  Core.Command.basic
    ...
    Core.Command.Let_syntax.(
      let%map_open
        input_file = anon ("input_file" %: string)
  ...
  Core.Command.basic
    ...
    Core.Command.Let_syntax.(
      let%map_open
        input_file = anon ("input_file" %: string)
        and output_file = anon ("output_file" %: string)
        and time = flag "-time" (optional_with_default 0. float)
          ~doc:"Time in the song to render."
      in
      fun () ->
        let _max_time, save_frame = frame_demo input_file in
  ...
  Core.Command.basic
    ...
    Core.Command.Let_syntax.(
      let%map_open
        ...
        and fps = flag "-fps" (required int)
          ~doc:" frames per second"
      in
      fun () ->
        let max_time, save_frame = frame_demo input_file in
        video save_frame max_time output_dir fps)

let trigger_cmd =
  Core.Command.basic
    ~summary:"Called on recompile."
    Core.Command.Let_syntax.(
      let%map_open
...
let command =
  Core.Command.group
    ~summary:"Do cool things with midis"
    [ "render", render_cmd
    ; "frame", frame_cmd
    ; "video", video_cmd
    ; "trigger", trigger_cmd
