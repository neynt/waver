...
let save ?(bit_depth = 16) ~sampling_rate filename (signals : Signal.t list) =
  let duration =
    List.map ~f:Signal.dur signals |> List.fold ~f:Float.max ~init:0.
  in
  let channels = List.length signals in
  let n =
    duration *. Float.of_int sampling_rate |> Float.round_up |> Int.of_float
  in
  let output_pt =
  ...
  |> List.iter ~f:(fun i ->
         ...
         List.iter signals ~f:(fun signal ->
             output_pt oc
               (Float.clamp_exn ~min:(-1.0) ~max:1.0 Signal.(signal.f t))));
  close_out oc
...
let load filename =
  ...
  let wav =
    ...
    let format_chunk =
      lift3
        ...
        ( string "fmt " *> chunk_size *> LE.any_uint16
        (* always 1, for "PCM" format *) *> LE.any_uint16 )
        (* num channels *)
        LE.any_int32 (* sampling rate *)
        ( LE.any_int32 (* data rate *) *> LE.any_uint16 (* bits per sample *)
        *> LE.any_uint16 )
      (* bit depth *)
    in

    let data_chunk =
      ( string "data" *> chunk_size >>= fun chunk_size ->
        take (Int32.to_int_exn chunk_size) )
    ...
    let chunk =
      choice ~failure_msg:"failed to parse any chunk"
  ...
  let { channels; sampling_rate; bit_depth } =
    List.find_map_exn chunks ~f:(function Format x -> Some x | _ -> None)
  in
  let samples =
    ...
    | 8 ->
        List.init channels ~f:(fun channel ->
            Array.init length ~f:(fun i ->
                Bigstring.unsafe_get_int8 samples ((i + channel) * stride)
                // 128))
    | 16 ->
        List.init channels ~f:(fun channel ->
            Array.init length ~f:(fun i ->
                Bigstring.unsafe_get_int16_le samples ((i + channel) * stride)
                // 32768))
    | 32 ->
        List.init channels ~f:(fun channel ->
            Array.init length ~f:(fun i ->
                (* TODO : maybe this doesn't in fact work? *)
                Bigstring.unsafe_get_int32_le samples ((i + channel) * stride)
                |> Obj.magic))
