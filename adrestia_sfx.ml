...
let lose_health =
  let open Signal in
  square 370. |> gain 0.08 |> chirp_exp 1.0 0.5 0.1
  |> mul (decay 0.2)
  |> crop 0.3
...
let fire_spell =
  ...
  |> phase_mod
       ( ramp
       |> add (sine 7. |> gain 0.0005 |> mul (pwlin [ (0.2, 1.0); (100., 1.0) ]))
       )
  |> mul (decay 0.2)
...
let render prefix =
  let save signal filename =
    ...
    Wav.save ~sampling_rate:44100
      (Caml.Filename.concat prefix filename)
      [ signal ]
  in
