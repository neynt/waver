...
let striped_rect (image : OImages.rgb24) x y w h stripe_width c1 c2 =
  for x = Int.max 0 x to Int.min (x + w - 1) (image#width - 1) do
    for y = Int.max 0 y to Int.min (y + h - 1) (image#height - 1) do
      let color = match (x + y) / stripe_width % 2 with 0 -> c1 | _ -> c2 in
      image#set x y color
    done
  done
...
let rect_gen (image : OImages.rgb24) x y w h style =
  ...
  | Striped (stripe_width, c1, c2) ->
      striped_rect image x y w h stripe_width c1 c2
...
let split start stop intervals =
  Array.map intervals ~f:(fun i ->
      start
      + (Float.of_int (stop - start) *. i |> Float.round_nearest |> Int.of_float))
...
let key_color midi =
  match midi % 12 with 1 | 3 | 6 | 8 | 10 -> `Black | _ -> `White

let piano ?(white_color = rgb 128 128 128) ?(black_color = rgb 0 0 0)
    ?(color_overrides = Map.empty (module Int)) ?(semitone_width = 21)
    ?(black_height = 100) ?(white_height = 150) ?(spacing = 1)
    (image : OImages.rgb24) min_note max_note =
  ...
  for midi = min_note to max_note do
    ...
    | `White ->
        ...
        let w =
          bot_start (white_idx + 1) - bot_start white_idx - (2 * spacing)
        in
