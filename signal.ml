...
(* The zero signal is zero for a zero time. *)
let zero =
  { f = (fun _t -> 0.)
  ; dur = 0.
  }
...
let cosine ?(amp=1.) freq =
  (fun t -> amp *. Float.cos (2. *. pi *. freq *. t))
  |> create_inf 
...
let integrate sr { f; dur } =
  ...
  in
  { f = (fun t -> aux (Int.of_float ((Float.round_nearest (t *. (Float.of_int sr))))))
  ; dur
  }
...
let echo delay { f; dur } =
  { f = (fun t -> (f t +. f (t +. delay)) /. 2.)
  ; dur
  }

let pwlin ?(y0=0.0) transitions =
  let fns, _, _ = List.fold transitions ~init:(Map.empty (module Float), 0.0, y0) ~f:(fun (fns, t, y1) (dur, y2) ->
    (* f(t) = mt + b *)
    let t' = t +. dur in
    let m = (y2 -. y1) /. dur in
    let b = y1 -. t *. m in
