...
let read_file filename =
  ...
  try
    let read_chunk ic tempo_map =
      if String.( <> ) (really_input_string ic 4) "MTrk"
      then failwith "Found a non-MTrk MIDI chunk";
      let _chunk_len = input_be_u32 ic in
      ...
      while not !fin do
        ...
        | 0, chan ->
          ...
          notes
            := List.cons
                 { time = start_time; dur = time -. start_time; midi; chan; velo }
                 !notes;
          ()
        | 1, chan ->
          let midi = input_byte ic in
        ...
        | 7, 15 ->
          ...
          | 0x59 ->
            ...
            key_signatures
              := (match
                    Map.add !key_signatures ~key:time ~data:{ accidentals; quality }
                  with
                 | `Ok x -> x
