...
let frame_demo input_file =
  ...
  let min_note, max_note =
    List.fold track ~init:(Int.max_value, Int.min_value) ~f:(fun (min, max) { midi; _ } ->
  ...
  let t_to_y t =
    ...
    - 150
    - Int.of_float
        (Float.min (Float.of_int Int.max_value_30_bits) (Float.round (t *. 120.)))
  in
  ...
  let save_frame time_ofs filename =
    ...
    Map.iteri midi_file.key_signatures ~f:(fun ~key ~data ->
      ...
      let scale =
        Scale.from_accidental_count
          ~quality:key_signature.quality
...
let video save_frame max_time output_dir fps =
  ...
  Stdlib.Printf.printf "Will generate %d frames.\n" max_frame;
  ...
  let rec loop frame =
    ...
    | false ->
      save_frame
        time_ofs
        (Core.Filename.concat output_dir (Stdlib.Printf.sprintf "%05d.png" frame));
      loop (frame + 1)
  in
  loop 0
