...
let button =
  let open Signal in
  noise () |> crop 0.2 |> mul (decay 0.02) |> gain 0.2
...
let lose_health =
  let open Signal in
  square 370. |> gain 0.08 |> chirp_exp 1.0 0.5 0.1 |> mul (decay 0.2) |> crop 0.3
...
let fire_spell =
  ...
  |> gain 0.2
  |> phase_mod
       (ramp |> add (sine 7. |> gain 0.0005 |> mul (pwlin [ 0.2, 1.0; 100., 1.0 ])))
  |> mul (decay 0.2)
  |> mul (pwlin [ 0.2, 1.0; 0.5, 0.0 ])
  |> crop 0.7

let trigger filename = Wav.save ~sampling_rate:44100 filename [ screen_transition ]
