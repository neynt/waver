...
type note =
  { content: [ `Pitch of scale_degree | `Rest ]
  ; length: int
  }
[@@deriving sexp]
...
let of_text_notation =
  ...
  let extender =
    (take_while (function '-' -> true | ' ' -> true | _ -> false)
     >>| String.count ~f:(function '-' -> true | _ -> false))
  in
  let integer = take_while1 (function '0' .. '9' -> true | _ -> false) >>| int_of_string in
  let note = lift3 (fun index accidental extender ->
    let accidental = match accidental with
      | `Sharp -> 1
      | `Flat -> -1
      | `Natural -> 0
    in
    { content = `Pitch { index; accidental }
    ; length = extender + 1
    })
    integer accidental extender
  in
  let melody = spaces *> many (lex note) in
  fun s ->
    parse_string melody s
