...
let plot_signal ?(lower = -1.) ?(upper = 1.) ?(resolution = 44100) (signal : Signal.t) w h output_file =
  ...
  let rec loop x prev_s =
    ...
    then ()
      ...
      in
      let y1 =
        List.min_elt pixel_samples ~compare:Float.compare
        |> Option.value_exn
        |> y_of_s
      and y0 =
        List.max_elt pixel_samples ~compare:Float.compare
        |> Option.value_exn
        |> y_of_s
      in
      let h = y1 - y0 + 1 in
...
let _main _input_file output_file =
  ...
  let _fft_signal =
    { discrete_signal with samples =
      fft discrete_signal.samples
      |> Array.map ~f:Caml.Complex.norm
    }
    |> Converter.zero_order_hold
  in
  plot_signal _fft_signal 1200 900 "signal.png"
;;
...
(*   let start = 10.0 in *)
  ...
  let save_frame time_ofs filename =
    ...
    in
(*     Wav.save ~sampling_rate:sample_rate output_file [ signal ]; *)
