...
let input_sbyte ic =
  ...
  match byte land 0x80 with 0 -> byte | _ -> byte - 0x100
...
let input_vlq ic =
  let rec aux cur =
    ...
    if b lsr 7 > 0 then aux cur else cur
  in
  aux 0
...
let read_file filename =
  ...
  if String.( <> ) (really_input_string ic 4) "MThd" then
    failwith "Not MIDI format";
  ...
  try
    let read_chunk ic tempo_map =
      if String.( <> ) (really_input_string ic 4) "MTrk" then
        failwith "Found a non-MTrk MIDI chunk";
      let _chunk_len = input_be_u32 ic in
      ...
      while not !fin do
        ...
        tick := !tick + ticks;
        let _, timing_fn =
          Option.value_exn (Map.closest_key !tempo_map `Less_than !tick)
        in
        ...
        | 7, 15 -> (
            ...
            | 0x51 ->
                ...
                tempo_map :=
                  Map.set !tempo_map ~key:start_tick ~data:(fun tick ->
                      start_time
                      +. Float.of_int ((tick - start_tick) * tempo)
                         /. (ticks_per_qn *. 1_000_000.))
            ...
            | 0x59 -> (
                ...
                key_signatures :=
                  match
                    Map.add !key_signatures ~key:time
                      ~data:{ accidentals; quality }
                  with
                  | `Ok x -> x
            ...
            | 0x2f ->
                ...
                fin := true
            | _ ->
                printf "[%f] Unknown meta 0x%x of length %d\n" time meta len;
                () )
        | _, chan ->
    ...
    let tempo_map =
      Map.add_exn tempo_map ~key:Int.min_value ~data:(fun ticks ->
          Float.of_int ticks *. 500_000. /. ticks_per_qn /. 1_000_000.)
    in
